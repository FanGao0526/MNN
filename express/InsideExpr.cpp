...
public:
    MergeExpr(const Optimizer::Merge* merge, int inputSize, int outputSize) : Solution(inputSize, outputSize) {
        ...
        {
            char numBuffer[20];
            char inputPrefix[] = "Input_", outputPrefix[] = "Output_";
            strcpy(numBuffer, inputPrefix);
            for (int i = 0; i < mInputs.size(); ++i) {
                sprintf(numBuffer + strlen(inputPrefix), "%d", i);
                schedule.inputTensors[std::string(numBuffer)] = mInputs[i].first;
            }
            strcpy(numBuffer, outputPrefix);
            for (int i = 0; i < mOutputs.size(); ++i) {
                sprintf(numBuffer + strlen(outputPrefix), "%d", i);
                schedule.outputTensor[std::string(numBuffer)] = mOutputs[i].first;
            }
        }
        //Create Backend
